...
export class ChatRepository {
   ...
   */
  async findDirectConversation(currentUserId: string, receiverId: string) {
   ...
   * Create Conversation
   */
  async createDirectConversation(currentUserId: string, receiverId: string) {
  ...
  async getUserConversations(userId: string) {
    const conversations = await prisma.conversation.findMany({
      where: {
        ...
        // Only show conversations that have at least
        // one non-deleted message
        messages: {
          some: {
            deletedAt: null,
          },
